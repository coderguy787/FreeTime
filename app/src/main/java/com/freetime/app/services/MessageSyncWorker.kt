...
// periodic sync of pending messages
class MessageSyncWorker(context: Context, params: androidx.work.WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        return try {
            Log.d(TAG, "Running message sync worker")

            try {
                OfflineMessageQueue.flush(applicationContext)
            } catch (e: Exception) {
                Log.e(TAG, "Failed to flush offline queue: ${e.message}")
            }
