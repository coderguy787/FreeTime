...
// periodic background polling for new messages
    ...
    private suspend fun pollNotifications() {
        try {
            ...
            val userId = prefs.getUserId() ?: run {
                Log.w(TAG, " User not logged in, skipping poll")
                return
            }
            ...
            if (response.isSuccess) {
                ...
                Log.d(TAG, " Got ${data.messages.size} messages")
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        try {
            ...
            if (response.isSuccess) {
                ...
                if (data != null) {
                    Log.d("NotificationPollingWorker", " Got ${data.messages.size} messages")
