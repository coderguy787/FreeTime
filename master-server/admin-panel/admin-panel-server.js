...
// self-signed cert on the api, skip verification
...
const API_PORT = process.env.PORT_API || 443;
...
app.get('/api/admin/monitor', adminComPort.middleware(), async (req, res) => {
    try {
        ...
        const [apiHealth, wsHealth, peerHealth, mongoHealth] = await Promise.all([
            ...
            checkServiceHealth('127.0.0.1', 27017, null)
        ]);
...
const proxyAdminRequests = async (req, res) => {
    ...
    try {
        ...
        // don't forward headers axios already handled
